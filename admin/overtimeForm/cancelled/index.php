...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h3 class="card-title">List of Cancelled Overtime Request</h3>
                ...
                <tbody>
                    ...
                    if (!empty($_settings->userdata('EMPNAME'))) {
                        if ($_settings->userdata('EMPPOSITION') == 5) {
                            $qry = $conn->query("SELECT * FROM overtime_requests WHERE `ot_status` = 4 ORDER BY `date_created` desc");
                        }
                        if ($_settings->userdata('EMPPOSITION') == 4) {
                            $qry = $conn->query("SELECT * FROM overtime_requests WHERE `ot_status` = 4 AND (`department` = '{$_settings->userdata('DEPARTMENT')}' 
                        ...
                        if ($_settings->userdata('EMPPOSITION') == 4) {
                            if ($_settings->userdata('DEPARTMENT') == 'Facilities/ MIS') {
                                $dept1 = "MIS";
                                $dept2 = "Facilities";
                                $qry = $conn->query("SELECT * FROM overtime_requests WHERE `ot_status` = 4 AND
                        ...
                        if ($_settings->userdata('EMPPOSITION') == 3) {
                            if ($_settings->userdata('DEPARTMENT') == 'PPC') {
                                $dept1 = "PPC";
                                $qry = $conn->query("SELECT * FROM overtime_requests WHERE `ot_status` = 4 AND `department` = '{$dept1}' ORDER BY `date_created` desc");
                            }
                        }
                        // if ($_settings->userdata('EMPPOSITION') == 3) {
                        ...
                        if ($_settings->userdata('EMPPOSITION') >= 2) {
                            $qry = $conn->query("SELECT * FROM overtime_requests WHERE `ot_status` = 4 AND `requestor_id` = '{$_settings->userdata('EMPLOYID')}' ORDER BY `date_created` desc");
                        }
                    } else {
                        $qry = $conn->query("SELECT * FROM overtime_requests WHERE `ot_status` = 4 ORDER BY `date_created` desc");
                    }
