            ...
            <table class="table table-bordered table-stripped">
                ...
                <tbody>
                    ...
                    if (!empty($_settings->userdata('EMPNAME'))) {
                        ...
                        if ($_settings->userdata('EMPPOSITION') >= 2) {
                            $qry = $conn->query("SELECT * FROM overtime_requests WHERE (`ot_status` = 0 OR `ot_status` = 1) AND `requestor_id` = '{$_settings->userdata('EMPLOYID')}' ORDER BY `date_created` desc");
                        }
                    } else {
                        $qry = $conn->query("SELECT * FROM overtime_requests WHERE (`ot_status` = 0 OR `ot_status` = 1) ORDER BY `date_created` desc");
                    }
