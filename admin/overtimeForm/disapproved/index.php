...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h3 class="card-title">List of Disapproved Overtime Request</h3>
                ...
                <tbody>
                    ...
                    if (!empty($_settings->userdata('EMPNAME'))) {
                        ...
                        if ($_settings->userdata('EMPPOSITION') == 4) {
                            $qry = $conn->query("SELECT * FROM overtime_requests WHERE `ot_status` = 3 AND (`department` = '{$_settings->userdata('DEPARTMENT')}' 
                        ...
                        if ($_settings->userdata('EMPPOSITION') == 4) {
                            if ($_settings->userdata('DEPARTMENT') == 'Facilities/ MIS') {
                                $dept1 = "MIS";
                                $dept2 = "Facilities";
                                $qry = $conn->query("SELECT * FROM overtime_requests WHERE `ot_status` = 3 AND
                        ...
                        if ($_settings->userdata('EMPPOSITION') == 3) {
                            if ($_settings->userdata('DEPARTMENT') == 'PPC') {
                                $dept1 = "PPC";
                                $qry = $conn->query("SELECT * FROM overtime_requests WHERE `ot_status` = 3 AND `department` = '{$dept1}' ORDER BY `date_created` desc");
                            }
                        }
                        // if ($_settings->userdata('EMPPOSITION') == 3) {
