<div class="card card-outline card-primary">
    <div class="card-header">
        <h3 class="card-title">List of Overtime Request</h3>
        <div class="card-tools">
            <?php
            if ($_settings->userdata('EMPPOSITION') == 2) {
            ?>
                <a href="<?php echo base_url ?>admin/?page=overtime_form/manage_ot" class="btn btn-flat btn-primary"><span class="fas fa-plus"></span> Create New</a>
            <?php
            }
            ?>
            <?php
                ...
                <tbody>
                    ...
                    if (!empty($_settings->userdata('EMPNAME'))) {
                        if ($_settings->userdata('EMPPOSITION') == 5) {
                            $qry = $conn->query("SELECT * FROM overtime_requests WHERE (ot_status = 1 OR od_name = '{$_settings->userdata('EMPLOYID')}') ORDER BY `date_created` desc");
                        }
                        if ($_settings->userdata('EMPPOSITION') == 4) {
                            $qry = $conn->query("SELECT * FROM overtime_requests WHERE `department` = '{$_settings->userdata('DEPARTMENT')}' 
                        ...
                        if ($_settings->userdata('EMPPOSITION') == 3) {
                            if ($_settings->userdata('DEPARTMENT') == 'PPC') {
                                $dept1 = "PPC";
                                $qry = $conn->query("SELECT * FROM overtime_requests WHERE `department` = '{$dept1}' ORDER BY `date_created` desc");
                            }
                        }
                        // if ($_settings->userdata('EMPPOSITION') == 3) {
                        ...
                        if ($_settings->userdata('EMPPOSITION') == 2) {
                            $qry = $conn->query("SELECT * FROM overtime_requests WHERE `requestor_id` = '{$_settings->userdata('EMPLOYID')}' ORDER BY `date_created` desc");
                        }
                    } else {
                        $qry = $conn->query("SELECT * FROM overtime_requests ORDER BY `date_created` desc");
                    }
                        ...
                        <tr>
                            ...
                            <td align="center">
                                ...
                                <div class="dropdown-menu" role="menu">
                                    <a class="dropdown-item" href="<?php echo base_url . 'admin?page=overtime_form/view_ot&id=' . $row['id'] ?>" data-id="<?php echo $row['id'] ?>"><span class="fa fa-eye text-dark"></span> View</a>
