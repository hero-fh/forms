                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <select name="emp_no" onchange="showHint(this.value)" id="emp_no" class="form-control select2" required>
                            ...
                            <?php
                            $aa = $conn->query("SELECT * FROM `employee_masterlist` where EMPLOYID !=0");
                            while ($row = $aa->fetch_assoc()) :
                            ?>
                                <option value="<?= $row['EMPLOYID'] ?>" <?= isset($emp_no) && $row['EMPLOYID'] == $emp_no ? 'selected' : '' ?>><?= $row['EMPLOYID'] . " - " . $row['EMPNAME'] ?></option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                    <div class="col-md-6">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <div class="input-group">
                                            <input autocomplete="off" type="text" class="form-control datepicker" id="date_commited" required name="date_commited[]" value="<?php echo isset($rows['date_commited'])  ? date('m/d/Y', strtotime($rows['date_commited'])) : '' ?>">
                                    ...
                                    <td>
                                        <input autocomplete="off" type="text" class="form-control" id="offense_no<?php echo $rows['id'] ?>" required name="offense_no[]" value="<?php echo isset($rows['offense_no'])  ? $rows['offense_no'] : '' ?>">
                                    ...
                                    <?php if ($ir_status < 3) { ?>
                                        <td>
                                            <a class="btn btn-sm btn-block dngr" <?php echo $irs->num_rows <= 1 ? 'style="pointer-events:none"' : '' ?> href="javascript:void(0)" data-id="<?php echo $rows['id'] ?>"><?php echo $irs->num_rows <= 1 ? '<span class="fas fa-ban text-danger">' : '<span class="fas fa-trash text-danger">' ?></span></a>
                                        </td>
                                    <?php } ?>
                                    <!-- <td>
...
<script>
    ...
    $('#ir-form').submit(function(e) {
        ...
        start_loader();
        $.ajax({
            url: _base_url_ + "classes/IR_DA_Master.php?f=re_pass_ir",
