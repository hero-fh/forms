        ...
        <div class="container-fluid">
            <table class="table table-bordered table-stripped">
                ...
                <tbody>
                    ...
                    //     $qry = $conn->query("SELECT * FROM ir_requests where (hr_status = 0) or (hr_status = 1 and sv_status = 0 and sv_name = " . $_settings->userdata('EMPLOYID') . ") ORDER BY `date_created` desc");
                    //     // $qry = $conn->query("SELECT * FROM ir_requests where ir_status = 2 ORDER BY `date_created` desc");
                    // } else {
                    $qry = $conn->query("SELECT * FROM ir_requests where hr_status = 1 and ir_status !=2 and ir_status != 3 and ir_status != 4 and requestor_id = " . $_settings->userdata('EMPLOYID') . " ORDER BY `date_created` desc");
                    // }
                        ...
                        <tr>
                            ...
                            <td align="center">
                                ...
                                <div class="dropdown-menu" role="menu">
                                    <a class="dropdown-item" href="<?php echo base_url . 'admin?page=incidentreport/completionIRDA/view_ir&id=' . md5($row['id']) ?>" data-id="<?php echo $row['id'] ?>"><span class="fa fa-eye text-dark"></span> View</a>
