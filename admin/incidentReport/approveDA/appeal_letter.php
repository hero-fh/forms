        ...
        <div class="row mb-3 mt-5">
            ...
            ?>
                ...
                <div class="col-md-2">
                    ...
                    <input readonly type="text" readonly name='code_no[]' class="form-control text-center" value="<?php echo $row['code_no'] ?>">
                </div>
                <div class="col-md-2">
                ...
                <div class="col-md-2">
                    <label class="control-label">Request no of days:</label>
                ...
                <div class="col-md-6">
                    <label class="control-label">Request suspension date: </label>
...
<?php } elseif ($appeal_status > 1) {
    $hr_mngr_name = $conn->query("SELECT EMPNAME FROM employee_masterlist WHERE EMPLOYID = $hr_mngr")->fetch_array();
    $has_od =  $conn->query("SELECT od_name FROM ir_requests WHERE ir_no = '{$ir_no}'")->num_rows;
    if ($has_od > 0) {
        $director_name = $conn->query("SELECT EMPNAME FROM employee_masterlist WHERE EMPLOYID ='{$od_name}'")->fetch_array();
    }
?>
    <div class="row justify-content-between">
        <div class="col-5">
            <label class="control-label text-info"><?php echo isset($appeal_status) && $appeal_status == 2 ? 'Validated' : 'Invalidated' ?> by :</label>
    ...
    <?php if ($appeal_status > 3) { ?>
        <div class="row justify-content-between">
            <div class="col-5">
                <label class="control-label text-info"><?php echo isset($appeal_status) && $appeal_status == 4 ? 'Approved' : 'Disapproved' ?> by :</label>
                <input readonly type="text" class="form-control" id="sv_name" value="<?php echo isset($director_name[0])  ? $director_name[0] : '' ?>">
                <i class="text-info" style="display:block; text-align: center;">Director</i>
            </div>
            <div class="col-4">
                <label class="control-label text-info">Date:</label>
                <input readonly type="text" class="form-control rounded-0" class="form-control form-control-sm rounded-0" value="<?php echo isset($od_sign_date)  ? date('m/d/Y h:i a', strtotime($od_sign_date)) : '' ?>">
            </div>
        </div>

    <?php } ?>
    <?php if ($appeal_status == 3 || $appeal_status == 5) { ?>
        <div class="col-md-12">
            <div class="form-group ">
