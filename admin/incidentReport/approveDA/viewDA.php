...
$approver_2 = $conn->query("SELECT APPROVER2 from employee_masterlist WHERE EMPLOYID= '{$emp_no}'")->fetch_array()[0];
$approver_1 = $conn->query("SELECT APPROVER1 from employee_masterlist WHERE EMPLOYID= '{$emp_no}'")->fetch_array()[0];
$approver_1 = $approver_1 == 'na' ? $approver_2 : $approver_1;
$approver_3 = $conn->query("SELECT APPROVER3 from employee_masterlist WHERE EMPLOYID= '{$emp_no}'")->fetch_array()[0];
                ...
                <div class="printable">
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        <?php if ($appeal_status != 4) { ?>
                            <tbody>
                                ...
                                <?php endwhile; ?>
                                <?php
                                $totalSum = $conn->query("SELECT sum(days_no) FROM `ir_list` WHERE `ir_no` = '$ir_no'and (da_type = 3 or da_type = 4 or da_type = 6)")->fetch_array()[0];
                                ?>
                                <tr>
                        ...
                        <?php } elseif ($appeal_status == 4) { ?>
                            <tbody>
                                ...
                                <?php endwhile; ?>
                                <?php
                                $totalSum = $conn->query("SELECT sum(appeal_days) FROM `ir_list` WHERE `ir_no` = '$ir_no'and (da_type = 3 or da_type = 4 or da_type = 6)")->fetch_array()[0];
                                ?>
                                <tr>
                                    <td colspan="6" class="text-right">TOTAL NO. OF DAYS(if suspension):</td>
                                    <td>
                    ...
                    <div class="row justify-content-between">
                        ...
                        <div class="col-4">
                            ...
                            <input readonly required type="date" name="da_requested_date" class="form-control  rounded-0" class="form-control  rounded-0" value="<?php echo isset($da_requested_date)  ? date('m/d/Y', strtotime($da_requested_date)) : date('Y-m-d') ?>">
                    ...
                    <div class="row justify-content-between">
                        ...
                        <?php } elseif ($da_status == 1 && $_settings->userdata('DEPARTMENT') == 'Human Resource' && $_settings->userdata('EMPPOSITION') == 4) { ?>
                            <div class="col-5">
                                <label class="control-label  text-info">For acknowledgement : </label>
                                <a href="javascript:void(0)" class="btn btn-block btn-success ack_data" data-id="<?php echo $id ?>" data-mngr="<?php echo $_settings->userdata('EMPLOYID') ?>" data-sign="2" data-emp="<?php echo $emp_no ?>">Acknowledge</a>
                                <i class="text-info" style="display:block; text-align: center;">HR Manager</i>
                            </div>
                            <div class="col-4">
                                <label class="control-label  text-info">Date </label>
                    ...
                    <div class="row justify-content-between">
                        ...
                        <?php } elseif ($da_status == 2 && $_settings->userdata('EMPLOYID') == $approver_1 && $_settings->userdata('EMPLOYID') != $approver_3) { ?>
                            <div class="col-5">
                                <label class="control-label  text-info">For acknowledgement : </label>
                                <a href="javascript:void(0)" class="btn btn-block btn-success ack_data" data-id="<?php echo $id ?>" data-sign="3" data-emp="<?php echo $emp_no ?>">Acknowledge</a>
                                <i class="text-info" style="display:block; text-align: center;">Approver 1</i>
                            </div>
                            <div class="col-4">
                                <label class="control-label  text-info">Date </label>
                                <input readonly required type="date" class="form-control  rounded-0" name="hr_mngr_sign_date" class="form-control  rounded-0" value="<?php echo date('Y-m-d') ?>">
                    ...
                    <div class="row justify-content-between">
                        <?php if ($da_status >= 4) {
                            ...
                            <div class="col-4">
                                <label class="control-label text-info">Date:</label>
                                <input readonly type="text" class="form-control rounded-0" class="form-control form-control-sm rounded-0" value="<?php echo isset($dm_sign_date)  ? date('m/d/Y h:i a', strtotime($dm_sign_date)) : '' ?>" required>
                            </div>
                        <?php } elseif (($da_status == 3 && $_settings->userdata('EMPLOYID') == $approver_3) || ($da_status == 2 && $_settings->userdata('EMPLOYID') == $approver_1 && $approver_1 == $approver_3)) { ?>
                            <div class="col-5 ack_btn">
                                <label class="control-label  text-info">For acknowledgement : </label>
                                <div class="button-wrapp text-center">
                                    <button class="btn btn-flat btn-lg btn-outline-success ack_data" type="button" data-id="<?php echo $id ?>" data-sign="4" data-emp="<?php echo $emp_no ?>"> <i class="fas fa-thumbs-up"></i> Acknowledge</button>
                                    <?php
                                    $is_suspend = $conn->query("SELECT da_type FROM `ir_list` where `ir_no` = '$ir_no' and valid = 1 and (da_type = 3 or da_type = 4 or da_type = 6)")->num_rows;

                                    if ($is_suspend > 0) { ?>
                                        <button class="btn btn-flat btn-lg btn-outline-warning appeal_data" type="button" data-id="<?php echo $id ?>" data-name="<?php echo $_settings->userdata('EMPLOYID') ?>"> <i class="fas fa-gavel"></i> Appeal</button>
                                    <?php } ?>
                                </div>
                    ...
                    <?php if ($appeal_status == 0) { ?>
                        <div class="row justify-content-end">
                            <button class="btn btn-flat  close_appeal_data btn-lg btn-outline-danger d-none " type="button" data-id="<?php echo $id ?>" data-val="2" data-sign="2" data-name="<?php echo $_settings->userdata('EMPLOYID') ?>"> <i class="fas fa-times"></i> Close</button>
                        </div>
                    <?php } ?>
                </div>
                <div id="trainContentDiv"></div>
                <?php if ($appeal_status > 0 && $approver_3 == $_settings->userdata('EMPLOYID')) { ?>
                    <div class="row justify-content-center">
                        <div class="col-5 ">
                            <label class="control-label  text-info">For acknowledgement : </label>
                            <div class="button-wrapp text-center">
...
<button class="btn btn-flat btn-lg btn-outline-warning click_1 appeal_data" style="opacity: 0; pointer-events:none;" type="button" data-id="<?php echo $id ?>" data-name="<?php echo $_settings->userdata('EMPLOYID') ?>"> <i class="fas fa-gavel"></i> Appeal</button>
