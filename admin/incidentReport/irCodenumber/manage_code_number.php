...
?>
		<center>Unknown code_number</center>
		<style>
			#uni_modal {
				display: none
			}
...
<div class="container-fluid">
	<form action="" id="code_number-form">
		...
		<div class="form-group">
			...
			<input readonly name="code_number" id="code_number" pattern="([À-ža-zA-Z0-9\s]+){2,}" type="text" class="form-control form-control-sm form-control-border" value="<?php echo isset($code_number) ? $code_number : ''; ?>">
		</div>
		<div class="form-group">
			<label for="violation" class="control-label">Violation</label>
			<textarea name="violation" id="violation" rows="10" class="form-control form-control-sm form-control-border"><?php echo isset($violation) ? $violation : ''; ?></textarea>
		</div>
		<div class="form-group">
...
<script>
	$(document).ready(function() {
		$('#uni_modal #code_number-form').submit(function(e) {
			...
			start_loader();
			$.ajax({
				url: _base_url_ + "classes/IR_DA_Master.php?f=code_number",
