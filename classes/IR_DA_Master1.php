...
class Master extends DBConnection
{
	...
	function save_ir_request() // saving ir to database: start
	{
		...
		if ($year = '2024') {
			$thisy = $this->conn->query("SELECT ir_no FROM ir_requests ORDER BY ir_no DESC LIMIT 1")->fetch_array()[0];
		...
		if (!empty($code_no)) {
			...
			foreach ($code_no as $k => $v) {
				...
				$violation[$k] = isset($violation[$k]) ? $violation[$k] : '';
				...
				$disposition[$k] = isset($disposition[$k]) ? $disposition[$k] : '';
	...
	function letter_of_explain() // saving ir to database: start
	{
		...
		$save = $this->conn->query($sql);
		if ($save) {
			$no_why =  $this->conn->query("SELECT id from `ir_list` where `why1` IS NULL and `emp_no` = '{$emp_no}' and `ir_no` = '{$ir_no}' ")->num_rows;
			// var_dump($hiodas);
			if ($no_why == 0) {
	...
	{
		extract($_POST);
		$del = $this->conn->query("UPDATE `ir_requests` set has_da = 1 where id = '{$id}' ");
		// $del = $this->conn->query("DELETE  FROM `ir_list` where id='{$id}'");
		if ($del) {
			$resp['status'] = 'success';
	...
	function ack_data()
	{
		...
		if ($sign == 2) {
			...
			$del = $this->conn->query("UPDATE `ir_requests` set da_status=3, sv_da_sign_date=NOW() where id = '{$id}' and emp_no  =  '{$emp_no}'");
		} elseif ($sign == 4) {
			$del = $this->conn->query("UPDATE `ir_requests` set da_status=4, dh_da_sign_date=NOW() where id = '{$id}' and emp_no  =  '{$emp_no}'");
		} elseif ($sign == 5) {
	...
	function appeal_ir() // saving ir to database: start
	{
		...
		foreach ($code_no as $k1 => $v1) {
			...
			$appeal_remarks = isset($appeal_remarks) ? $appeal_remarks : '';
