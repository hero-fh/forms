...
$fields = array('EMP. NO.', 'EMP. NAME ', 'DATE REQUESTED', 'DATE START', 'TIME START', 'TIME END', 'No. of Hours', 'REMARKS ', 'DATE APPROVED '); 
...
if($qry->num_rows > 0){ 
  ...
  while($row = $qry->fetch_assoc()){ 
    ...
    $dateApproved = date('m-d-Y h:m a', strtotime($row['dh_sign_date']));
    ...
    if (mysqli_num_rows($qry1) != 0){
      while($row1 = $qry1->fetch_assoc()):
        ...
        $lineData = array($row1['emp_num'], $row1['emp_name'], $dateRequested, $dateFrom, $timeFrom, $timeTo, $hours,$row1['ot_reason'], $dateApproved); 
